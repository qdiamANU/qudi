...
channels can be sacrificed to create up to 3 digital output channels:
...
class AWGSpectrumM4i6631x8(Base, PulserInterface):
    ...
    def get_constraints(self):
        ...
        activation_config['config9'] = {'a_ch1', 'a_ch2'}
    ...
    def reset(self):
        ...
        if spcm_dwSetParam_i32(self._hCard, SPC_M2CMD, M2CMD_CARD_RESET):
            self._read_out_error()
            self.log.info('AWG reset error!')
            return -1
        else:
            self.log.info('AWG reset!')
            self.connected = True
            return 0
